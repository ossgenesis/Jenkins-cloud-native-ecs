...
Use below command to build new jenkins controller:
```shell
...
cd docker
```
Edit `Dockerfile.agent` jenkins version and add required package:
```yaml
...
Run terraform after push agent and controller in ECR:
```bash
terraform init -reconfigure -backend-config='bucket=jenkins-ecs-statefile-tf' -backend-config='key=devops/terraform.tfstate' -backend-config='region=eu-west-1'
terraform plan "-var-file=terraform.tfvars" "-out=tfplan"
terraform apply "tfplan"
