...
```bash
terraform init -reconfigure -backend-config='bucket=jenkins-ecs-statefile-tf' -backend-config='key=devops/terraform.tfstate' -backend-config='region=eu-west-1'
...
### Upgrade Jenkins controller and agents:
...
The entry point is also overridden to fetch the configuration from S3 if the variable `JENKINS_CONF_S3_URL` is defined.
...
Later follows the remaining steps:
```
...
docker tag <new build image id or name> 136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-aws-fargate:2.479.3

docker push 136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-aws-fargate:2.479.3
```

You can pull it from Docker Hub: [Official jenkins docker images](https://hub.docker.com/r/jenkins/jenkins/tags?name=2.479).

##### Jenkins agents: Dockerfile. agent

Image for Jenkins agents. We install some packages as an example. Note that all Jenkins agent images must derive
...
cd docker
```
Edit `Dockerfile.agent` Jenkins version and add the required package:
```yaml
# This is just an example of a Jenkins agent
# Use the latest compatible inbound agent version for Jenkins 2.479.3
FROM --platform=linux/amd64 jenkins/inbound-agent:latest-alpine-jdk17
# Use root user to install packages
USER root

# Install required packages, including Terraform
...
Run Terraform after pushing the agent and controller in ECR:
```bash
terraform init -reconfigure -backend-config='bucket=jenkins-ecs-statefile-tf' -backend-config='key=devops/terraform.tfstate' -backend-config='region=eu-west-1'
terraform plan "-var-file=terraform.tfvars" "-out=tfplan"
terraform apply "tfplan"
...
Process of building new agent will be same as mentioned above section `Jenkins agents: Dockerfile.agent`
However, we need to update Jenkins `JCASC`, so jenkins will accept new agent after a restart:
For instance, currently taking an example of `Dockerfile.kaniko` agent. 
Once you done with build and push image in ECR. You need to update following files in `modules, jcasc template, variables and main.tf`:

**Inside module folder level:**

[jcasc template](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/modules/ecs-jenkins/templates/jcasc.template.yml#lines-101:127)

[configuration-as-code.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/modules/ecs-jenkins/configuration-as-code.tf#lines-17:18,53,58,57,56,55,54)

[soci.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/modules/ecs-jenkins/soci.tf#lines-30:35)

[variables.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/modules/ecs-jenkins/soci.tf#lines-30:35)


**Root folder level**:

[main.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/main.tf#lines-43)

[variables.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/variables.tf#lines-154:163)

[terraform.tf](https://bitbucket.org/its-ait-confluent/ait-jenkins-ecs-iac/src/ca7a788cf6f5fa9ce081ca630fdf8e40ba38e2a8/terraform.tfvars#lines-20)
...
terraform apply "tfplan"
```
Validate, agent at `jenkins UI > Manage Jenknis > Cloud > fargate ` [URL](https://awsjenkins02.woolworths.co.za/manage/cloud/fargate/configure)
