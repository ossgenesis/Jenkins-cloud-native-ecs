...
resource "aws_ecr_repository" "glue_agent" {
  count                = var.soci.enabled ? 1 : 0
  name                 = "jenkins-alpine-agent-aws-glue"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
}

resource "aws_ecr_repository" "lambda_agent" {
  count                = var.soci.enabled ? 1 : 0
  name                 = "jenkins-alpine-agent-aws-lambda"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
}

resource "aws_ecr_repository" "kaniko_agent" {
  count                = var.soci.enabled ? 1 : 0
  name                 = "jenkins-alpine-agent-aws-kaniko"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
}

resource "aws_ecr_repository" "ace_agent" {
  count                = var.soci.enabled ? 1 : 0
  name                 = "jenkins-alpine-agent-aws-ace"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
}

resource "aws_ecr_repository" "terraform_agent" {
  count                = var.soci.enabled ? 1 : 0
  name                 = "jenkins-alpine-agent-aws-terraform"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
}
