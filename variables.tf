...
variable "controller_docker_image" {
  ...
  default     = "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-aws-fargate:2.462"
...
variable "agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: devops ECR"
  default     = "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-alpine-agent-aws:2.479.3"
...
variable "glue_agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: https://hub.docker.com/r/jenkins/inbound-agent/"
  default     = "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-alpine-agent-aws:2.479.3"

  validation {
    condition     = can(regex("^.*:.*$", var.glue_agent_docker_image))
    error_message = "The glue_agent_docker_image variable must be in the form <image>:<tag>"
  }
}

variable "lambda_agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: https://hub.docker.com/r/jenkins/inbound-agent/"
  default     = "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-alpine-agent-aws:2.479.3"

  validation {
    condition     = can(regex("^.*:.*$", var.lambda_agent_docker_image))
    error_message = "The lamdba_agent_docker_image variable must be in the form <image>:<tag>"
  }
}

variable "ace_agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: https://hub.docker.com/r/jenkins/inbound-agent/"
  default     =  "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-alpine-ace-agent-aws:v1"

  validation {
    condition     = can(regex("^.*:.*$", var.ace_agent_docker_image))
    error_message = "The ace_agent_docker_image variable must be in the form <image>:<tag>"
  }
}

variable "terraform_agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: https://hub.docker.com/r/jenkins/inbound-agent/"
  default     =  "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-alpine-terraform-agent-aws:v1"

  validation {
    condition     = can(regex("^.*:.*$", var.terraform_agent_docker_image))
    error_message = "The terraform_agent_docker_image variable must be in the form <image>:<tag>"
  }
}

variable "kaniko_agent_docker_image" {
  type        = string
  description = "Docker image to use for the example agent. See: https://hub.docker.com/r/jenkins/inbound-agent/"
  default     =  "136474465872.dkr.ecr.eu-west-1.amazonaws.com/ww/jenkins-aws-fargate-kaniko:latest"

  validation {
    condition     = can(regex("^.*:.*$", var.kaniko_agent_docker_image))
    error_message = "The kaniko_agent_docker_image variable must be in the form <image>:<tag>"
  }
}
...
EOF
  type = object({
    ...
    index_builder_image = optional(string, "public.ecr.aws/j4w9x2o6/ww/soci-index-builder:latest") # Index builder image to use
  })
  default = {}
}
