def call(String branchName) {
    switch (branchName) {
        ...
        case 'pre-prod':
            // terraform state file
            env.S3_BUCKET_STATE = "ww-ait-euw1-sg-pre-prod-iac-tf.state-s3"
            // s3 bucket path to store the state
            // env.S3_BUCKET_KEY = "ait-lambda/preprod/terraform.tfstate"
            env.AWS_REGION = "eu-west-1"
            env.DYNAMODB_TABLE = "woolworths_ait_sg_preprod_tf_state"
            break
        case 'main':
            // terraform state file
            env.S3_BUCKET_STATE = "ww-ait-euw1-sg-iac-tf.state-s3"
            // s3 bucket path to store the state
            // env.S3_BUCKET_KEY = "ait-lambda/prod/terraform.tfstate"
            env.AWS_REGION = "eu-west-1"
            env.DYNAMODB_TABLE = "woolworths_ait_sg_tf_state"
            break
        default:
